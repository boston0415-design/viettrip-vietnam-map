...
window.google={maps:{Marker:TestMarker,Circle:TestMarker,Size:class{},Point:class{},event:{trigger:empty},places:{AutocompleteSessionToken:class{},AutocompleteService:class{getPlacePredictions(r,cb){window.lastAutocompleteQuery=r.input;cb([{place_id:'external-ux',structured_formatting:{main_text:'검색한 새 업소',secondary_text:'호치민 주소'}}],'OK')}},PlacesService:class{getDetails(r,cb){cb({place_id:r.placeId,name:r.placeId==='reading-google'?'한식당 호미도 1군 HOMIDO BBQ Quận 1':r.placeId==='flower-google'?'Blossom 꽃집':r.placeId==='google-wax-a'?'Waxing A':'검색한 새 업소',formatted_address:'정확한 주소',geometry:{location:{lat:()=>10.775,lng:()=>106.705}},types:r.placeId==='flower-google'?['florist','store']:['cafe'],photos:r.placeId==='reading-google'?[{getUrl(){return 'https://fixture.invalid/photo1.png'},html_attributions:[]}]:undefined,formatted_phone_number:'+84 123 456 789',website:r.placeId==='flower-google'?'https://flower.fixture.invalid':undefined,opening_hours:{weekday_text:r.placeId==='reading-google'?['월','화','수','목','금','토','일'].map(day=>day+'요일 09:00–21:00'):['월요일 09:00–21:00'],isOpen:()=>true},rating:4.5,user_ratings_total:22},'OK')}findPlaceFromQuery(r,cb){cb([],'ZERO_RESULTS')}}}}};
window.aiQuestionCalls=0;window.aiGoogleRequests=[];
window.google.maps.places.Place=class{static async searchByText(request){window.aiGoogleRequests.push(request);if(/^(?:flower bouquet )?florist /.test(request.textQuery))return {places:[{id:'flower-google',displayName:'Blossom 꽃집',types:['florist','store'],formattedAddress:'Quận 1, Hồ Chí Minh',location:{lat:10.779,lng:106.702},rating:4.8,userRatingCount:100},{id:'flower-cafe',displayName:'꽃 장식 카페',types:['cafe','store'],formattedAddress:'Quận 1, Hồ Chí Minh',location:{lat:10.779,lng:106.702},rating:5,userRatingCount:1000}]};if(/^ramen /.test(request.textQuery))return {places:[{id:'ramen-google',displayName:'SHODAY RAMEN',types:['restaurant','ramen_restaurant'],formattedAddress:'Quận 1, Hồ Chí Minh',location:{lat:10.779,lng:106.702},rating:4.8,userRatingCount:100},{id:'ramen-google-wrong',displayName:'중식 국수 전문점',types:['restaurant','chinese_restaurant'],editorialSummary:'국수·라멘',formattedAddress:'Quận 1, Hồ Chí Minh',location:{lat:10.779,lng:106.702},rating:5,userRatingCount:900}]};if(/affordable|^restaurant nice atmosphere/.test(request.textQuery))return {places:[['rank-local-google','가격 비교 회원 식당','EXPENSIVE',5,''],['rank-cheap-google','저렴한 Google 식당','INEXPENSIVE',4.2,''],['rank-mood-google','분위기 좋은 Google 식당','MODERATE',4.5,'Great atmosphere and lovely interior.'],['rank-unknown-google','가격 미확인 Google 식당',null,4.9,'Bad atmosphere.']].map(([id,displayName,priceLevel,rating,editorialSummary])=>({id,displayName,priceLevel,rating,editorialSummary,userRatingCount:50,types:['restaurant'],formattedAddress:'Quận 1, Hồ Chí Minh',location:{lat:10.779,lng:106.702},businessStatus:'OPERATIONAL',attributions:[]}))};if(request.textQuery.includes('Phu My Hung'))return {places:[['g-menu-fish','Google Sashimi',''],['g-menu-bbq','BBQ Grill',''],['g-menu-review','Google 메뉴 후기','사시미를 먹었습니다.'],['g-menu-wrong','노출 금지 해산물','']].map(([id,displayName,review])=>({id,displayName,types:['restaurant','vietnamese_restaurant'],reviews:review?[{text:review,authorAttribution:{displayName:'메뉴 후기 작성자',uri:'https://maps.google.com/reviewer'},googleMapsURI:'https://maps.google.com/review'}]:[],rating:4.8,userRatingCount:55,formattedAddress:'Tân Phong, Quận 7, Hồ Chí Minh',location:{lat:10.72975,lng:106.71302},businessStatus:'OPERATIONAL',attributions:[]}))};if(request.textQuery.includes('Korean'))return {places:[['google-room-known','별실 안내 Google 한식당','korean_restaurant','Private dining rooms available.','2'],['google-room-unknown','룸 정보 없는 Google 한식당','korean_restaurant','','2'],['google-room-no','룸 없는 Google 한식당','korean_restaurant','No private rooms.','2'],['google-room-wrong','노출 금지 Google 베트남','vietnamese_restaurant','Private rooms.','2'],['google-room-d7','노출 금지 Google 7군','korean_restaurant','Private rooms.','7']].map(([id,displayName,type,editorialSummary,district])=>({id,displayName,editorialSummary:id==='google-room-known'?'':editorialSummary,reviews:id==='google-room-known'?[{text:'프라이빗 룸이 있습니다.',authorAttribution:{displayName:'Google 후기 작성자',uri:'https://maps.google.com/reviewer'},googleMapsURI:'https://maps.google.com/review',relativePublishTimeDescription:'한 달 전'}]:[],types:[type,'restaurant'],rating:4.8,userRatingCount:100,formattedAddress:'Quận '+district+', Hồ Chí Minh, Vietnam',location:{lat:10.803,lng:106.732},businessStatus:'OPERATIONAL',currentOpeningHours:{periods:[{open:{day:0,hour:0,minute:0}}]},attributions:[]}))};if(request.textQuery.includes('French'))return {places:[['google-fr-a','French Table','french_restaurant'],['google-fr-b','Maison Test','french_restaurant'],['google-fr-korean','잘못된 한식 Google','korean_restaurant'],['google-fr-viet','잘못된 베트남 Google','vietnamese_restaurant'],['google-fr-unknown','French 제목만 있는 식당','restaurant']].map(([id,displayName,type])=>({id,displayName,types:[type,'restaurant'],rating:4.9,userRatingCount:90,formattedAddress:'Quận 1, Hồ Chí Minh, Vietnam',location:{lat:10.779,lng:106.702},businessStatus:'OPERATIONAL',addressComponents:[{types:['country'],shortText:'VN'}],attributions:[]}))};if(!request.textQuery.includes('waxing'))return {places:[]};return {places:[['google-wax-registered','회원 왁싱샵',4.1,10,'12'],['google-wax-a','Waxing A',4.9,100,'12'],['google-wax-b','Waxing B',4.9,20,'12'],['google-wax-low','낮은 평점',3.8,500,'12'],['google-wax-wrong','다른 군',5,200,'11']].map(([id,displayName,rating,userRatingCount,district])=>({id,displayName,rating,userRatingCount,formattedAddress:'Quận '+district+', Hồ Chí Minh, Vietnam',location:{lat:10.867,lng:106.654},businessStatus:'OPERATIONAL',types:['beauty_salon'],addressComponents:[{types:['country'],shortText:'VN'}],attributions:[]}))};}constructor({id}){this.id=id;}async fetchFields(){const p=testData.places.find(p=>p.googlePlaceId===this.id);if(!p)throw Error('NOT_FOUND');Object.assign(this,{displayName:p.name,formattedAddress:p.address,location:{lat:()=>p.lat,lng:()=>p.lng},businessStatus:'OPERATIONAL',currentOpeningHours:{periods:[{open:{day:0,hour:0,minute:0}}]},attributions:[]});}};
fetch=async(url,options={})=>{if(String(url).includes('/api/answer-map')){window.aiAnswerCalls=(window.aiAnswerCalls||0)+1;const data=JSON.parse(options.body);return {ok:true,json:async()=>({picks:data.candidates.slice(0,2).map(c=>({id:c.id,reasons:['match','rating']})),basis:'ai'})};}if(String(url).includes('/api/ask-map')){window.aiQuestionCalls++;const query=JSON.parse(options.body).query;if(query.includes('정모'))return {ok:false,json:async()=>({error:'Intent provider deliberately unavailable'})};if(query.includes('분짜가 뭐가 달라'))return {ok:true,json:async()=>({intent:{mode:'advice',answer:'퍼는 국물에 쌀국수를 넣어 먹고, 분짜는 구운 돼지고기와 쌀국수를 소스에 곁들여 먹는 음식이에요.',terms:[],unsupported:[],preferences:[]}})};if(query.includes('꽃을 선물'))return {ok:true,json:async()=>({intent:{relevant:true,city:'hcmc',category:'shopping',subcategory:'',service:'florist',flowerGift:true,terms:['꽃집'],unsupported:[],preferences:[],requestText:query}})};if(query.includes('라멘'))return {ok:true,json:async()=>({intent:{relevant:true,city:'hcmc',category:'restaurant',subcategory:'',terms:['라멘'],unsupported:[],preferences:['top_rated'],sortBy:'top_rated',requestText:query}})};if(query.includes('쌀국수'))return {ok:true,json:async()=>({intent:{relevant:true,city:'hcmc',category:'restaurant',subcategory:'',terms:['쌀국수'],unsupported:[],preferences:['top_rated'],sortBy:'top_rated',pickOne:true,requestText:query}})};if(query.includes('꼰선섬'))return {ok:true,json:async()=>({intent:{relevant:true,city:'hcmc',terms:[],unsupported:[],preferences:[],requestText:query,travelDestination:query.includes('껀터의 Cồn Sơn')?'conson-cantho':'conson-choice'}})};if(query.includes('달랏')&&query.includes('교통편'))return {ok:true,json:async()=>({intent:{relevant:true,city:'hcmc',category:'',terms:[],unsupported:[],transport:{origin:'hcmc',destination:'dalat',mode:'all',originExplicit:true}}})};if(query.includes('아이폰 듀오'))return {ok:true,json:async()=>({intent:{relevant:true,city:'hcmc',category:'shopping',terms:[],unsupported:[],preferences:[],productSearch:true,action:'purchase',productName:'아이폰 듀오',requestText:query}})};if(query.includes('햄버거'))return {ok:true,json:async()=>({intent:{relevant:true,city:'hcmc',category:'restaurant',terms:['햄버거'],unsupported:[],preferences:[],action:'grabfood'}})};const hanoi=query.includes('하노이'),date=query.includes('여자친구'),french=query.includes('프랑스'),room=query.includes('룸'),fish=query.includes('횟집'),meat=query.includes('고기집'),cheap=query.includes('저렴한 식당'),mood=query.includes('분위기 좋은 식당');return {ok:true,json:async()=>({intent:{relevant:true,city:hanoi?'hanoi':'hcmc',district:room?'2':query.includes('왁싱')?'12':date?'1':'',area:fish||meat?'푸미흥':'',category:room||french||fish||meat||cheap||mood?'restaurant':query.includes('왁싱')?'spa':hanoi?'restaurant':date?'bar':'cafe',subcategory:room?'한식':french?'프랑스':'',features:room?['private_room']:[],terms:fish?['회']:meat?['고기·구이']:query.includes('동태탕')?['동태탕']:query.includes('왁싱')?['왁싱']:[],sortBy:cheap?'cheap':mood?'atmosphere':'',showPrice:cheap,preferences:cheap?['cheap']:mood?['atmosphere']:date?['date','atmosphere']:[],visitToday:query.includes('오늘'),benefit:/혜택|제휴/.test(query),recommended:query.includes('강추'),nearby:false,unsupported:[]}})}};if(String(url).includes('/api/product-info'))return {ok:true,json:async()=>({status:'upcoming',source:'https://www.apple.com/vn/iphone-duo/',releaseDate:'2026-10-23',checkedAt:'2026-09-24T00:00:00Z',fresh:true})};if(String(url).includes('api.met.no'))return {ok:true,headers:{get:()=>new Date(Date.now()+3600000).toUTCString()},json:async()=>({properties:{timeseries:[{time:new Date(Math.floor(Date.now()/3600000)*3600000).toISOString(),data:{instant:{details:{air_temperature:27}},next_1_hours:{summary:{symbol_code:'cloudy'}}}}]}})};let req;try{req=JSON.parse(options.body)}catch{};const data=['badges','moderation'].includes(req?.p_action)?[]:{id:'ux-member',nickname:'시험 회원',total:3,level:1,devices:[{hash:'a'.repeat(64),key:'test-device-key'}],counts:{place:1,review:2,correction:0},activities:[],corrections:[]};return {ok:true,json:async()=>data,text:async()=>'',status:200};};
`;
const source=fs.readFileSync(path.join(root,'index.html'),'utf8');
const html=source.replace(/<script[^>]+src="[^"]*(?:online-presence|realtime-|community-stats|10-runtime-guard|home-screen|guide-map-link|transport-guide|admin-regions)[^"]*"[^>]*><\/script>/g,'').replace(/(<script src="\.\/assets\/js\/09-init-events[^>]*>)/,`<script>${fixture}</script>$1`);
...
(async()=>{
 ...
 for(const width of [320,390,768,1440]){
  ...
  async function checkDetailReading(label){
   ...
   await page.locator('#detail>.detailResizeHandle').press('ArrowDown');
   const before=await panel.evaluate(p=>{p.scrollTop=80;const r=p.getBoundingClientRect();return {height:r.height,top:r.top,scroll:p.scrollTop};});
   assert.equal(before.scroll,80,label+' has enough real content to read');
   async function swipe(delta){
    const point=await panel.evaluate(p=>{const r=p.getBoundingClientRect(),h=p.querySelector('.detailHeader').getBoundingClientRect();return {x:r.left+Math.min(r.width/2,120),y:Math.max(h.bottom+24,r.top+r.height/2)};});
    const {x,y}=point;
    if(width<901){
     const cdp=await context.newCDPSession(page);
     await cdp.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[{x,y}]});
     for(let i=1;i<=8;i++)await cdp.send('Input.dispatchTouchEvent',{type:'touchMove',touchPoints:[{x,y:y-delta*i/8}]});
     await page.waitForTimeout(160);await cdp.send('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[]});await cdp.detach();
  ...
  await checkDetailReading('member');
  await page.evaluate(()=>setDetailExpanded(true));
  ...
  assert(await page.locator('#operatorDialog').isVisible());
  assert.equal(await page.locator('#memberDialog').isVisible(),false);
  assert(await page.locator('#operatorPlaces').isVisible());
  assert(await page.locator('#memberModerationLoad').isVisible());
  assert.match(await page.locator('#memberModeration').innerText(),/접수된 정보 수정 제안이 없습니다/);
  assert.equal(await page.locator('#operatorDialog .menuResizeGrip').count(),0);
  const before=await page.locator('#operatorDialog').evaluate(n=>n.getBoundingClientRect().height);
  await page.locator('#operatorDialog .operatorBody').evaluate(n=>n.scrollTop=180);
  assert.equal(await page.locator('#operatorDialog').evaluate(n=>n.getBoundingClientRect().height),before);
  await page.locator('#operatorDialog .operatorBody').evaluate(n=>n.scrollTop=0);
  await checkDoubleClick('#operatorDialog','#operatorTitle');
  await page.screenshot({path:`test-results/operator-${width}.png`});
  await page.locator('#operatorClose').click();
  ...
  await page.evaluate(()=>PlaceSearch.openGoogle({placeId:'reading-next',name:'다음 Google 장소'}));
  assert.equal(await page.locator('#detail').evaluate(p=>p.scrollTop),0,'switching Google places starts from the top');
  ...
  await page.locator('#browseShowList').click();
  assert(await page.locator('#businessSide').isVisible(),'reproduce search with an existing list');
  const beforeSearchScope=await page.evaluate(()=>JSON.stringify([state.city,state.cat,state.sub,state.ratingFilter,state.benefitFilter,state.nearby]));
  await page.locator('#aiMapQuestion').focus();
  assert(await page.locator('#aiMapExamples').isVisible());
  assert(!await page.locator('#businessSide').isVisible(),'AI search closes the old business list on every layout');
  assert(!await page.locator('#areaLegend').isVisible(),'nearby toolbar cannot cover AI results');
  assert.equal(await page.evaluate(()=>JSON.stringify([state.city,state.cat,state.sub,state.ratingFilter,state.benefitFilter,state.nearby])),beforeSearchScope,'search focus preserves browsing filters');
  await page.goBack();await page.waitForFunction(()=>document.getElementById('aiMapPanel').hidden);
  assert(!await page.locator('#businessSide').isVisible(),'Back does not resurrect an unrelated list');
  await page.locator('#aiMapQuestion').focus();
  ...
  assert(!await page.locator('#businessSide').isVisible(),'loaded results still own the reading space');
  ...
  await page.locator('#aiMapQuestion').focus();
  assert(!await page.locator('#detail').isVisible(),'a new search closes the previous detail');
  await page.locator('#searchInput').fill('혜택 있는 카페');
  ...
  assert.equal(await page.locator('#aiMapPanel').isVisible(),false);
  await page.evaluate(()=>closeDetailPanel());
  // The reported ramen query must not match the combined registration tag.
  await page.evaluate(()=>testData.places.push(...[
   {id:'ramen-doya',name:'Doya Jjambbong 도야짬뽕',tags:['국수·라멘','강추업소'],memberBenefit:true,description:'중국집'},
   {id:'ramen-wok',name:'기름진 멜로 Wok of love',tags:['국수·라멘'],description:'퓨전 중국 요리집'},
   {id:'ramen-member',name:'회원 RAMEN 식당',tags:['국수·라멘']},
   {id:'ramen-mixed',name:'혼합 메뉴 식당',tags:['국수·라멘'],description:'돈코츠 라멘 판매'}
  ].map(p=>({category:'restaurant',subcategory:'중식',address:'Quận 1, Hồ Chí Minh',lat:10.779,lng:106.702,initialRating:5,...p}))));
  await page.locator('#aiMapQuestion').fill('라멘.가장.맛있는 집 찾아줘');
  await page.locator('#aiMapQuestion').press('Enter');
  await page.waitForFunction(()=>document.querySelector('#aiMapTitle').textContent==='추천 업소 · 3곳'&&document.querySelectorAll('.aiGoogleResult').length===1);
  assert.deepEqual((await page.locator('.aiMemberResult').evaluateAll(nodes=>nodes.map(n=>n.dataset.placeId))).sort(),['ramen-member','ramen-mixed']);
  assert.equal(await page.locator('.aiGoogleResult').getAttribute('data-google-place-id'),'ramen-google');
  assert(!/도야짬뽕|기름진 멜로|중식 국수 전문점|국수·라멘/.test(await page.locator('#aiMapResults').innerText()));
  assert.equal(await page.locator('#nearbyResults').isVisible(),false,'search results own the active panel');
  await page.screenshot({path:path.join(out,`ai-ramen-${width}.png`)});
  const ramenCard=page.locator('[data-place-id="ramen-member"]');if(width<901)await ramenCard.tap();else await ramenCard.click();
  assert.match(await page.locator('#detail').innerText(),/회원 RAMEN 식당/);
  assert.equal(await page.locator('#aiMapPanel').isVisible(),false);
  await page.evaluate(()=>{closeDetailPanel();testData.places=testData.places.filter(p=>!p.id.startsWith('ramen-'));});
  await page.evaluate(()=>testData.places.push({id:'flower-member',name:'회원 꽃집',category:'shopping',address:'Quận 1, Hồ Chí Minh',lat:10.779,lng:106.702,initialRating:5,memberBenefit:true,tags:['강추업소']}));
  await page.locator('#aiMapQuestion').fill('여자친구에게 꽃을 선물해주고 싶어');
  await page.locator('#aiMapQuestion').press('Enter');
  await page.waitForFunction(()=>document.querySelector('#aiMapTitle').textContent==='추천 업소 · 2곳'&&document.querySelectorAll('.aiGoogleResult').length===1);
  assert.equal(await page.locator('.aiMemberResult').getAttribute('data-place-id'),'flower-member');
  assert.equal(await page.locator('.aiGoogleResult').getAttribute('data-google-place-id'),'flower-google');
  assert(!/꽃 장식 카페|AI의 일반 안내/.test(await page.locator('#aiMapPanel').innerText()));
  assert.match(await page.locator('#aiMapNote').innerText(),/공개된 전화·웹사이트/);
  assert.equal(await page.locator('#nearbyResults').isVisible(),false);
  await page.screenshot({path:path.join(out,`ai-florist-${width}.png`)});
  const flowerCard=page.locator('[data-google-place-id="flower-google"]');if(width<901)await flowerCard.tap();else await flowerCard.click();
  await page.waitForFunction(()=>document.querySelector('#detail .externalCall'));
  assert.match(await page.locator('#detail').innerText(),/Blossom 꽃집/);
  assert.equal(await page.locator('#detail .externalCall').getAttribute('href'),'tel:+84123456789');
  assert.equal(await page.locator('#detail a[href="https://flower.fixture.invalid/"]').count()+await page.locator('#detail a[href="https://flower.fixture.invalid"]').count(),1);
  assert.equal(await page.locator('#aiMapPanel').isVisible(),false,'a result tap switches to contact/details');
  await page.evaluate(()=>{closeDetailPanel();testData.places=testData.places.filter(p=>p.id!=='flower-member');});
  await page.evaluate(()=>testData.places.push({id:'rank-member',googlePlaceId:'rank-local-google',name:'가격 비교 회원 식당',category:'restaurant',subcategory:'한식',address:'Quận 1, Hồ Chí Minh',lat:10.779,lng:106.702,initialRating:5}));
  await page.locator('#aiMapQuestion').fill('호치민에서 저렴한 식당 찾아줘');await page.locator('#aiMapQuestion').press('Enter');
  await page.waitForFunction(()=>!document.querySelector('#aiMapResults').classList.contains('aiRankingPending')&&document.querySelector('.aiResult')?.dataset.googlePlaceId==='rank-cheap-google');
  assert.match(await page.locator('[data-place-id="rank-member"] .aiPrice').innerText(),/높은 가격대/);
  assert.equal(await page.locator('.aiResult').first().getAttribute('data-google-place-id'),'rank-cheap-google','price outranks membership');
  await page.screenshot({path:path.join(out,`ai-price-${width}.png`)});
  await page.locator('#aiMapQuestion').focus();assert.equal(await page.locator('.aiResult').first().getAttribute('data-google-place-id'),'rank-cheap-google','refocus keeps enriched prices');
  await page.locator('#aiMapQuestion').fill('1군에서 분위기 좋은 식당 찾아줘');await page.locator('#aiMapQuestion').press('Enter');
  await page.waitForFunction(()=>document.querySelector('.aiResult')?.dataset.googlePlaceId==='rank-mood-google');
  assert.match(await page.locator('.aiResult').first().innerText(),/Great atmosphere/);
  await page.screenshot({path:path.join(out,`ai-atmosphere-${width}.png`)});
  const ranked=page.locator('.aiResult').first();if(width<901)await ranked.tap();else await ranked.click();
  assert.equal(await page.locator('#aiMapPanel').isVisible(),false,'ranked card opens detail directly');
  await page.evaluate(()=>closeDetailPanel());
  ...
  await page.evaluate(()=>{testData.places.push({id:'alias-onsi',name:'Onsi Spa',category:'spa',subcategory:'스파',address:'Nha Trang',area:'나트랑',lat:12.24,lng:109.19});});
  await page.locator('#searchInput').fill('온시 스파');
  await page.locator('.placeSearchOption').filter({hasText:'Onsi Spa'}).click();
  assert.match(await page.locator('#detail .detailName').innerText(),/Onsi Spa/);
  await page.evaluate(()=>closeDetailPanel());
  await page.locator('#aiMapQuestion').fill('호치민에서 달랏으로 가는 교통편 찾아줘');await page.locator('#aiMapQuestion').press('Enter');
  await page.waitForFunction(()=>document.querySelectorAll('.aiTravelAnswer').length===3);
  assert.match(await page.locator('#aiMapTitle').innerText(),/호치민 → 달랏/);
  assert.match(await page.locator('#aiMapResults').innerText(),/달랏 직통 없음/);
  assert.match(await page.locator('#aiMapNote').innerText(),/실시간 시간표.*조회한 결과는 아닙니다/);
  const travelBox=await page.locator('#aiMapPanel').boundingBox();assert(travelBox.x>=-1&&travelBox.x+travelBox.width<=width+1,'travel results fit the viewport');
  await page.screenshot({path:path.join(out,`ai-travel-${width}.png`)});
  await page.locator('.aiTravelPlace').first().click();
  assert.match(await page.evaluate(()=>window.lastAutocompleteQuery),/tan son nhat/i,'airport query reaches Google autocomplete');
  assert.match(await page.locator('#detail h2').innerText(),/검색한 새 업소/,'resolved airport result opens the persistent detail panel');
  assert.equal(await page.locator('#aiMapPanel').isVisible(),false);
  await page.evaluate(()=>closeDetailPanel());
  await page.evaluate(()=>testData.places.push({id:'delivery-burger',name:'샘플 햄버거',category:'restaurant',subcategory:'퓨전',description:'수제 햄버거',address:'1 Test, Ho Chi Minh',area:'호치민',lat:10.77,lng:106.7}));
  await page.locator('#aiMapQuestion').fill('맛있는 햄버거집 찾아서 그랩푸드로 연결해줘');await page.locator('#aiMapQuestion').press('Enter');
  await page.waitForFunction(()=>document.querySelectorAll('.aiActionBox').length>0);
  assert.equal(await page.locator('.aiMemberResult button,.aiMemberResult a').count(),0,'action links cannot be nested in the detail button');
  const grab=page.locator('.aiActionBox a').first();assert.equal(await grab.getAttribute('href'),'https://food.grab.com/vn/en/');
  assert.equal(await grab.getAttribute('target'),'_blank');
  await page.locator('.aiActionCopy').first().click();assert.equal(await page.locator('#aiMapPanel').isVisible(),true,'copy does not open detail/close results');
  const actionBox=await page.locator('.aiActionBox').first().boundingBox();assert(actionBox.x>=0&&actionBox.x+actionBox.width<=width+1);
  await page.screenshot({path:path.join(out,`ai-delivery-${width}.png`)});
  await page.locator('#aiMapQuestion').fill('호치민에서 아이폰 듀오 가장 저렴한 매장 찾아줘');await page.locator('#aiMapQuestion').press('Enter');
  await page.waitForFunction(()=>document.querySelector('.aiProductAnswer')?.textContent.includes('2026-10-23'));
  assert.equal(await page.locator('.aiResult').count(),0,'product answer never shows shopping mall candidates');
  assert.equal(await page.locator('.aiProductAnswer a').first().getAttribute('href'),'https://www.apple.com/vn/iphone-duo/');
  await page.screenshot({path:path.join(out,`ai-product-${width}.png`)});
  await page.locator('.aiProductAnswer button').click();assert.equal(await page.locator('#aiMapQuestion').inputValue(),'호치민에서 아이폰 듀오 가장 저렴한 매장 찾아줘','editing preserves question');
  await page.locator('#aiMapClear').click();assert.equal(await page.locator('.aiProductAnswer').count(),0,'clear removes product results');
  await page.screenshot({path:path.join(out,`overview-${width}.png`)});
  assert.deepEqual(errors,[],`no runtime error at ${width}px`);
  await page.locator('#aiMapQuestion').fill('꼰선섬 어떻게 가야해?');
  await page.locator('#aiMapSend').click();
  await page.getByRole('button',{name:'껀터의 꼰선섬 · Cồn Sơn',exact:true}).click();
  await page.getByRole('heading',{name:'껀터 꼰선섬 · Cồn Sơn 가는 방법',exact:true}).waitFor();
  assert.equal(await page.locator('#aiMapResults .aiMemberResult').count(),0);
  assert.equal(await page.locator('#aiMapResults .aiGoogleResult').count(),0);
  assert.match(await page.locator('#aiMapResults').innerText(),/꼬박 선착장/);
  assert(!(await page.locator('#aiMapResults').innerText()).includes('사이공강에서 보내는 한때'));
  await page.screenshot({path:`test-results/ai-destination-${width}.png`});
  await page.locator('#aiMapClear').click();
  await page.evaluate(()=>testData.places.push({id:'answer-pho',name:'Phở 테스트',category:'restaurant',subcategory:'베트남',area:'호치민',address:'Quận 1, Hồ Chí Minh',lat:10.779,lng:106.702,initialRating:4.8}));
  await page.locator('#aiMapQuestion').fill('호치민 쌀국수 원탑은?');await page.locator('#aiMapQuestion').press('Enter');
  await page.locator('.aiAnswerPick').first().waitFor();
  assert.match(await page.locator('.aiAnswer').innerText(),/한 곳을 고르면/);
  assert.match(await page.locator('.aiAnswer').innerText(),/Phở 테스트/);
  assert.equal(await page.locator('.aiResult').count(),1);
  const answerBounds=await page.locator('.aiAnswer').boundingBox();assert(answerBounds.x>=0&&answerBounds.x+answerBounds.width<=width+1);
  const answerCalls=await page.evaluate(()=>window.aiAnswerCalls);
  await page.locator('#aiMapQuestion').focus();assert.equal(await page.evaluate(()=>window.aiAnswerCalls),answerCalls,'refocus reuses answer without another paid call');
  await page.screenshot({path:`test-results/ai-answer-${width}.png`});
  if(width<901)await page.locator('.aiAnswerPick').first().tap();else await page.locator('.aiAnswerPick').first().click();
  assert.equal(await page.locator('#aiMapPanel').isVisible(),false);assert.match(await page.locator('#detail').innerText(),/Phở 테스트/);
  await page.evaluate(()=>closeDetailPanel());
  await page.locator('#aiMapQuestion').fill('퍼와 분짜가 뭐가 달라?');await page.locator('#aiMapQuestion').press('Enter');
  await page.waitForFunction(()=>document.getElementById('aiMapTitle').textContent==='AI 답변');
  assert.equal(await page.locator('.aiResult').count(),0);assert.match(await page.locator('.aiAnswer').innerText(),/구운 돼지고기/);
  await page.screenshot({path:`test-results/ai-advice-${width}.png`});
  await page.locator('#aiMapClear').click();assert.equal(await page.locator('.aiAnswer').count(),0);
  // The real reported question must continue when both AI intent providers fail.
  await page.evaluate(()=>{state.city='hcmc';testData.places.push(
    {id:'context-group',name:'단체 모임 식당',description:'단체 모임 안내 있습니다.',category:'restaurant',address:'Quận 1, Hồ Chí Minh',lat:10.779,lng:106.702,initialRating:4.6},
    {id:'context-cafe',name:'대화 카페',category:'cafe',address:'Quận 1, Hồ Chí Minh',lat:10.779,lng:106.702,initialRating:4.4}
  );});
  const contextBefore=await page.evaluate(()=>JSON.stringify(testData));
  await page.locator('#aiMapQuestion').fill('내일 정모인데 모일만한 장소 추천해줘');await page.locator('#aiMapQuestion').press('Enter');
  await page.locator('[data-place-id="context-group"]').waitFor();
  assert.match(await page.locator('.aiSearchContext').innerText(),/정모·단체 모임/);
  assert.equal(await page.locator('.aiContextOptions button').count(),3);
  assert.equal(await page.locator('.aiHours').count(),0,'tomorrow never displays today hours');
  const contextBounds=await page.locator('.aiSearchContext').boundingBox();assert(contextBounds.x>=0&&contextBounds.x+contextBounds.width<=width+1);
  await page.screenshot({path:path.join(out,`ai-context-recovery-${width}.png`)});
