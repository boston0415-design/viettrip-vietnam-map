...
(async()=>{
 ...
 for(const width of [320,390,768,1440]){
  ...
  await page.evaluate(()=>{testData.places.push(...Array.from({length:12},(_,i)=>({id:'ai-hanoi-'+i,name:'하노이 등록 식당 '+i,category:'restaurant',subcategory:'베트남',area:'하노이',address:'Hà Nội, Việt Nam',lat:21.03,lng:105.84,initialRating:i%2?5:4,memberBenefit:i===0,benefitText:i===0?'회원 음료 혜택':'',tags:[]})),{id:'ai-bar-mood',name:'데이트 바',category:'bar',subcategory:'루프탑 바',area:'호치민',address:'Quận 1, Hồ Chí Minh',lat:10.779,lng:106.702,description:'분위기가 좋고 데이트하기 좋아요',initialRating:4},{id:'ai-bar-plain',name:'일반 바',category:'bar',subcategory:'펍',area:'호치민',address:'Quận 1, Hồ Chí Minh',lat:10.779,lng:106.702,initialRating:5});});
  await page.locator('#aiMapQuestion').fill('하노이에서 회원들이 강추한 식당 찾아줘');
  await page.locator('#aiMapQuestion').press('Enter');
  await page.locator('.aiResult').first().waitFor();
  assert.equal(await page.locator('.aiResult').count(),12,'all same-city registered alternatives are listed');
  assert.match(await page.locator('#aiMapTitle').innerText(),/대신.*12곳/);
  ...
  assert.match(await page.locator('#aiMapStatus').innerText(),/오늘 영업 여부는 방문 전 확인/);
