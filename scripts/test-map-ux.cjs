...
(async()=>{
 ...
 for(const width of [320,390,768,1440]){
  ...
  results.push({width,passed:true,checks:['matching search design, date preferences, strict cuisine and benefit eligibility, verified-room and inquiry groups, truthful badges and stable detail','list-only quick filters','no public dashboard counts','operator controls in list options','desktop double-click max/min and mobile drag preservation','readable list under half height','expand-first title drag with anchored header and grip','nearby collapse/close/reopen','drag-to-collapse','title to detail','previous/next','photo drag versus tap','review close and frozen save target','photo modal','list/nearby restoration','benefit text','membership height','search registration','manual location validation']});
  await context.close();
 }
