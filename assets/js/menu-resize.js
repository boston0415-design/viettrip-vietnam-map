(() => {
 ...
 function limits(p){
   ...
   const max=Math.max(80,available-32);return {min:Math.min(150,max),max};
 }
 ...
 function scan(){
   for(const [selector,header] of configs)for(const p of document.querySelectorAll(selector)){
     if(!records.has(p)){
       ...
       grip.title=p.id==='businessSide'?'손잡이를 끌어 높이 조절 · 제목과 목록은 함께 스크롤':'손잡이 또는 제목을 끌어 높이 조절 · 본문은 스크롤';grip.innerHTML='<span aria-hidden="true"></span>';p.prepend(grip);p.classList.add('menuResizable');records.set(p,{grip});
       window.BodySheetDrag?.bind(p,{
         handlesOnly:true,includeHeaders:true,headerSelector:p.id==='businessSide'?'.menuResizeGrip':header+',.menuResizeGrip',bounds:()=>records.get(p).bounds||limits(p),
