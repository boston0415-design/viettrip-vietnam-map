...
const MAP_SYMBOL_PATHS={
  ...
  "massage": "M308-620C384-672 528-660 620-592C684-544 684-456 616-396C544-336 536-264 536-188C536-108 488-60 412-60C320-60 276-124 276-208C276-304 228-412 228-500C228-552 252-592 308-620ZM206-788C206-729.46 241.82-682 286-682C330.18-682 366-729.46 366-788C366-846.54 330.18-894 286-894C241.82-894 206-846.54 206-788ZM402-828C402-789.34 427.97-758 460-758C492.03-758 518-789.34 518-828C518-866.66 492.03-898 460-898C427.97-898 402-866.66 402-828ZM550-766C550-731.76 572.39-704 600-704C627.61-704 650-731.76 650-766C650-800.24 627.61-828 600-828C572.39-828 550-800.24 550-766ZM656-676C656-647.28 675.7-624 700-624C724.3-624 744-647.28 744-676C744-704.72 724.3-728 700-728C675.7-728 656-704.72 656-676ZM720-544C720-519.7 736.12-500 756-500C775.88-500 792-519.7 792-544C792-568.3 775.88-588 756-588C736.12-588 720-568.3 720-544Z",
...
function personalPlaceActionsHtml(p){
  ...
  return `<div class="cardPersonalActions"><button type="button" class="cardRecommend" data-personal-action="recommend" data-place-id="${esc(p.id)}" aria-label="${esc(p.name)} 강추 선택·수정">${icon('<path d="M7 10v11H3V10h4Zm0 0 4-7a2 2 0 0 1 2 2v4h6a2 2 0 0 1 2 2l-1 8a2 2 0 0 1-2 2H7"/>')}<span>강추</span></button><button type="button" data-personal-action="favorite" data-place-id="${esc(p.id)}" class="${favorite?'isFavorite':''}" aria-pressed="${favorite}" aria-label="${esc(p.name)} 즐겨찾기 ${favorite?'해제':'추가'}" title="즐겨찾기 ${favorite?'해제':'추가'}">${icon(star)}</button><button type="button" data-personal-action="hide" data-place-id="${esc(p.id)}" aria-label="${esc(p.name)} ${hidden?'숨기기 해제':'내 목록에서 숨기기'}" title="${hidden?'숨기기 해제':'내 목록에서 숨기기'}">${icon(eye)}</button></div>`;
}
