...
function personalPlaceActionsHtml(p){
  ...
  return `<div class="cardPersonalActions"><button type="button" data-personal-action="favorite" data-place-id="${esc(p.id)}" class="${favorite?'isFavorite':''}" aria-pressed="${favorite}" aria-label="${esc(p.name)} 즐겨찾기 ${favorite?'해제':'추가'}" title="즐겨찾기 ${favorite?'해제':'추가'}">${icon(star)}</button><button type="button" class="cardRecommend" data-personal-action="recommend" data-place-id="${esc(p.id)}" aria-label="${esc(p.name)} 강추 선택·수정">👍 강추</button><button type="button" data-personal-action="hide" data-place-id="${esc(p.id)}" aria-label="${esc(p.name)} ${hidden?'숨기기 해제':'내 목록에서 숨기기'}" title="${hidden?'숨기기 해제':'내 목록에서 숨기기'}">${icon(eye)}</button></div>`;
}
