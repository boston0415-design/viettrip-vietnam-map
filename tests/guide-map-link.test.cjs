...
(async()=>{
 ...
 const stay=await fixture('?from=guide&city=hcmc&category=stay&benefit=1');stay.run(`assert.equal(state.city,'hcmc');assert.equal(state.cat,'stay');assert.equal(state.navCategory,'stay');assert.equal(state.benefitFilter,'benefit');assert.equal(state.ratingFilter,'all');assert.deepEqual(items().map(p=>p.id),['h1']);assert.equal(window.personalView,'all');assert.equal(loads,1)`);assert.match(stay.w.document.getElementById('relatedGuideLink').href,/read=member-benefits/);stay.close();
