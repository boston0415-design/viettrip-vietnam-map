...
const files=fs.readdirSync(path.join(__dirname,'../assets/js')).filter(f=>/^(0[1-8]|10)-/.test(f)&&f.endsWith('.js')).sort();
...
renderHierarchyNav=()=>{};syncMapFilterSummary=()=>{};clearAreaLabels=()=>{};hideHover=()=>{};
...
for(const city of Object.keys(CITY_DATA)){
 for(const def of NAV_CATEGORIES){
  ...
  const expected=def.kind==='business'?def.id:({shopping:'shopping','market-nav':'market','attraction-nav':'attraction','golf-nav':'golf',hospital:'hospital',pharmacy:'pharmacy'})[def.id];
  const result=items();
