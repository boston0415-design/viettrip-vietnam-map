...
const pin=businessMarkerIcon('stay','호텔',4.5);const svg=decodeURIComponent(pin.url.split(',')[1]);assert(svg.includes('stroke-width="1.5"'));assert(svg.includes('<circle'));assert(!svg.includes('4.5'));
console.log('PASS every fixed POI and range hover/tap info, no unnecessary coordinate/range text, escaped content, all 10 category glyphs, compact circular markers');
`,c);
