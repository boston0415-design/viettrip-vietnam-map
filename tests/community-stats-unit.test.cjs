...
async function load({fail=false,blocked=false,sessionBlocked=false,host='viettrip-vietnam-map.pages.dev'}={}){
 ...
 const context={Intl,AbortController,Date:{now:()=>now},setTimeout,clearTimeout,SUPABASE_URL:'https://test.invalid',SUPABASE_HEADERS:{},location:{hostname:host},crypto:{randomUUID},localStorage:storage(local,blocked),sessionStorage:storage(session,sessionBlocked),document:{getElementById:k=>nodes[k]},fetch:async(url,options)=>{
