...
(async()=>{
 ...
 data.places.push(place('funeral','근조 꽃집'));
 for(const name of ['근조 꽃집','Funeral Flowers','Hoa Viếng Hoa Tang','Hoa Đám Tang Quận 1'])assert(!w.AIMapSearch.flowerPurposeMatches(name,intent),name);
 assert(w.AIMapSearch.flowerPurposeMatches('Hoa Tặng Người Yêu',intent),'gift and funeral Vietnamese accents must not be conflated');
 assert(w.AIMapSearch.flowerPurposeMatches('Funeral Flowers',{...intent,flowerGift:false}),'non-gift florist searches are not restricted');
 ...
 records.push(raw('funeral','Shop Hoa Đám Tang Hoa Viếng Quận 1'));
 ...
 assert.equal(request.includedType,'florist');assert.equal(request.useStrictTypeFiltering,true);assert.match(request.textQuery,/^flower bouquet florist Ho Chi Minh City Vietnam$/);assert(!/cafe|girlfriend|gift|date|shopping/.test(request.textQuery));
