...
(async()=>{
 ...
 for(const text of ['룸이 있는지 모르겠어요','프라이빗룸 있나요?','룸 여부 문의 필요','조용한 데이트 맛집','Private room plans are not confirmed','Private rooms may be available','룸을 원했어요','쇼룸 구경했어요'])assert.equal(classify(text),'unknown',text);
 ...
 const summary=w.AIGoogleSearch.rowsFrom([raw('summary','Korean restaurant with private rooms.')],intent)[0];assert.equal(summary.room.evidence,'Google 업소 설명 · Korean restaurant with private rooms.');
 const records=[raw('google-known','',{reviews:[{text:'We enjoyed our dinner in a private room.',authorAttribution:{displayName:'Room Reviewer',uri:'https://maps.google.com/reviewer'},googleMapsURI:'https://maps.google.com/review',relativePublishTimeDescription:'한 달 전'}]}),raw('google-unknown'),raw('Google private room name only'),raw('google-no','No private rooms.'),raw('google-other','Private rooms.',{types:['vietnamese_restaurant']}),raw('google-d7','Private rooms.',{formattedAddress:'Quận 7, Hồ Chí Minh'})];
 ...
 for(const request of requests){assert.equal(request.includedType,'korean_restaurant');assert.equal(request.useStrictTypeFiltering,true);assert(request.locationRestriction);assert(request.fields.includes('currentOpeningHours'));assert(request.fields.includes('editorialSummary'));assert(request.fields.includes('reviews'));}
 ...
 assert.equal(w.document.querySelector('.aiRoomCredit a').textContent,'Room Reviewer');assert.equal(w.document.querySelector('.aiRoomCredit a').href,'https://maps.google.com/reviewer');
