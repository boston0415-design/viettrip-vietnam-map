...
(async()=>{
 ...
 const records=[raw('g-fish','Google Sashimi'),raw('g-review','Google 현지 메뉴',{reviews:[{text:'사시미를 먹었습니다.',authorAttribution:credit,googleMapsURI:'https://maps.google.com/review'}]}),raw('g-wrong','노출 금지 해산물',{types:['restaurant','seafood_restaurant','japanese_restaurant']}),raw('g-no','노출 금지',{editorialSummary:'No sashimi available.'}),raw('g-bbq','BBQ Grill'),raw('g-type','구이 타입',{types:['restaurant','barbecue_restaurant']}),raw('g-fr','혼합 French 메뉴',{reviews:[{text:'French cuisine is served here.',authorAttribution:credit}]})];
